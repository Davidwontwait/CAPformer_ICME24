...
def main():
    ...
    for val_data in val_loader:
        ...
        psnr0 += util.calculate_psnr(dark_og, rlt_img)
        ssim0 += util.calculate_ssim(dark_og, rlt_img)
        lpips0 += util.calculate_lpips(dark_og, rlt_img)

    psnr /= len(val_loader)
    ssim /= len(val_loader)
    lpips /= len(val_loader)

    psnr0 /= len(val_loader)
    ssim0 /= len(val_loader)
    lpips0 /= len(val_loader)

    logger.info(f'OG Metirc: PSNR: {psnr:.4f} SSIM: {ssim:.4f} LPIPS: {lpips:.4f} ')
    logger.info(f'Beginning Metirc: PSNR: {psnr0:.4f} SSIM: {ssim0:.4f} LPIPS: {lpips0:.4f} ')
            ...
            #### validation
                if  valid:
                    ...
                    for val_data in val_loader:
                        ...
                        fea_0 = visuals['fea_0']
                        # mask = util.tensor2img(visuals['mask0'])
                        ...
                        #     util.save_fea_map(img, lq_img, save_img_path)
                        #
                        #
                        # save_img_path = os.path.join(img_dir,
                        #                              f'{img_name}.png')
                    ...
                    log_s = '# Current Iter: {} Validation # PSNR: {:.4f}: SSIM: {:.4f}'.format(current_step,
                                                                                                psnr_total_avg,
                                                                                                ssim_total_avg)
                    logger.info(log_s)
                    ...
                    if ssim_total_avg > best_ssim:
                        ...
                        for i_name,img in imgs.items():
                            ...
                            save_img_path = os.path.join(save_img_path, f'{i_name}.png')
                            util.save_img(img, save_img_path)
                    log_s = f'best psnr:{best_psnr:.4f} at iter {best_psnr_i} , ssim: {best_ssim:.4f} at iter {best_ssim_i}'
                    logger.info(log_s)
