...
from models.loss import CharbonnierLoss2, VGGLoss,SSIM, ColorLoss,MS_SSIM_L1_LOSS,PSNRLoss
from models.ours import ours as ours
...
class enhancement_model(BaseModel):
                ...
                for optimizer in self.optimizers:
                    self.schedulers.append(
                        lr_scheduler.CosineAnnealingRestartCyclicLR(
                            optimizer, train_opt['T_period'], eta_mins=[train_opt['lr_G'], 1e-6], ))
            else:
    ...
    def loss(self,x1,x2):
        ...
        if self.opt_loss['ssim']:
            l_ssim = self.cri_ssim(x1, x2)*0.01
            loss += l_ssim
    ...
    def feed_data(self, data, need_GT=True):
        ...
        for i in range(len(self.var_L)):
            source = self.GT[i]
            target = self.var_L[i]
            mean_brightness_source = torch.mean(source.float())
            mean_brightness_target = torch.mean(target.float())
            source = source * (mean_brightness_target/mean_brightness_source)
            self.GT[i] = source

        if need_GT:
            # self.real_H = data['med'].to(self.device)
            self.real_H = self.GT
